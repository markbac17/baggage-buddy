...
import DeleteIcon from "@material-ui/icons/LocalShipping";
...
const EnhancedTableToolbar = (props) => {
  ...
  return (
    <Toolbar className={clsx(classes.root, { [classes.highlight]: numSelected > 0 })}>
      ...
      {numSelected > 0 ? (<Tooltip title="For Delivery"><IconButton onClick={(e) => DisplayDeliveryList(selected)}><DeleteIcon /></IconButton></Tooltip>
...
function DisplayDeliveries(props) {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = props.file.map((n) => n.file_id);
      ...
      <Paper className={classes.paper}>
        ...
        <TableContainer>
          <Table className={classes.table} aria-labelledby="tableTitle" size={dense ? "medium" : "small"}>
            ...
            <TableBody>
              {stableSort(props.file, getComparator(order, orderBy))
                .slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
                .map((p, index) => {
                  const isItemSelected = isSelected(p.file_id);
              ...
              {emptyRows > 0 && (
                <TableRow style={{ height: (dense ? 53 : 33) * emptyRows }}>
                  <TableCell colSpan={6} />
                </TableRow>
              )}
            </TableBody>
        ...
        <TablePagination rowsPerPageOptions={[5, 10, 20]} component="div" count={props.file.length} rowsPerPage={rowsPerPage}
